...
GUI 启动时会自动检测两台后端的实际运行状态。若两台均已部署，可直接
点击“远程运行”；运行前还会执行一次快速复检。工具栏中的“清理远端”
可以手动清理容器、OVS 网桥、接收进程和运行状态。由当前 GUI 会话完成
的部署会在正常退出时自动清理；启动前已存在且属于其他会话的部署不会被
退出流程误删。

部署结果按主机记录；即使只有一台部署成功，该主机仍属于当前会话，退出时会
尝试清理。关闭窗口会异步等待工作线程退出，再执行带会话校验的清理，界面会
保持响应；清理失败会显示原因。部署、健康检查和清理命令的执行期限分别默认
为 600、20、120 秒，可通过 `SATNET_DEPLOY_TIMEOUT_SEC`、
`SATNET_HEALTH_TIMEOUT_SEC`、`SATNET_CLEANUP_TIMEOUT_SEC` 覆盖。

远程运行前还会校验两端数据集的运行编号、配置摘要、主机归属，以及本地的
高度、倾角、相位、历元、拓扑和时间片周期。数据集片数必须足够覆盖本次运行。
播放从数据集记录的起始时间开始。

使用已导出的实验时，选择“数据 -> 载入数据集配置”，打开该数据集的
`manifest.json`，可恢复星座、历元、拓扑、时间片周期和片数。将同一次导出的
`hosts/gzy0/`、`hosts/gzy1/` 中的内容分别放到对应后端的
`satnet-backend/runtime/data/`，包括各自的 `manifest.json`。
后端也需要更新本仓库的 `scripts/health.sh`。旧清单缺少实验元信息时，
可以继续检测和清理资源，但不能通过运行校验，需要重新导出数据集。
...
新清单还包含 `experiment` 和 `config_digest`：前者保存完整的轨道、历元、
起始时间、拓扑及随机失效参数，后者是该配置的 SHA-256 摘要。
全局清单和两端主机清单携带相同的实验配置与摘要。
...
指标显示区分“中断”“无数据”“数据异常”“查询失败”。`NaN`、无穷值、
负时延和越界丢包率会被标记为数据异常，不参与排名或图表数值计算。
